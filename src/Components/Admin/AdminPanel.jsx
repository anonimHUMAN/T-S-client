...
function AdminPanel() {
    ...
    const subPass = async () => {
        if (inp.current.value === '' || inp1.current.value === '' || inp2.current.value === '') {
            ...
            let res = await axios.post(`${config.url}/admin/password`, data, {
                headers: {
            ...
            <div className={`App ${darkMode ? 'dark' : 'light'}`}>
                <main className="bg-white dark:bg-gray-900 h-screen">
                    <div className="text-center fixed top-5 right-6">
                        <button onClick={togglePass} className="inline-block text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 focus:ring-4 focus:outline-none focus:ring-gray-200 dark:focus:ring-gray-700 rounded-lg text-sm p-1.5" type="button">
